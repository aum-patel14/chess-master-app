...
import CustomEngineWorker from '../workers/stockfish.worker?worker';
import { BotConfig } from '../config/bots';

export interface CustomEngineHook {
  isThinking: boolean;
  getBestMove: (fen: string, config: BotConfig) => Promise<string>;
}

export const useCustomEngine = (): CustomEngineHook => {
  const [isThinking, setIsThinking] = useState(false);
  const workerRef = useRef<Worker | null>(null);
  const resolveRef = useRef<((value: string) => void) | null>(null);
  const rejectRef = useRef<((reason: any) => void) | null>(null);
  // Track isThinking in a ref so useCallback never goes stale
  const isThinkingRef = useRef(false);

  useEffect(() => {
    let worker: Worker;
    try {
      worker = new CustomEngineWorker();
      workerRef.current = worker;
    } catch (e) {
      console.error('Failed to spawn Custom AI Engine Worker:', e);
      return;
    }
    ...
    worker.onmessage = (event) => {
      ...
      // Ignore the 'ready' message — don't treat it as a bestmove
      ...
      if (data.type === 'bestmove') {
        ...
        if (resolveRef.current) {
          // Resolve with empty string (not null) so caller gets a string
    ...
    worker.onerror = (err) => {
      console.error('Custom AI worker error:', err);
      setIsThinking(false);
      isThinkingRef.current = false;
  ...
  const getBestMove = useCallback((fen: string, config: BotConfig): Promise<string> => {
    return new Promise((resolve, reject) => {
      if (!workerRef.current) {
        reject(new Error('Custom AI worker is not initialized.'));
        return;
      }
      ...
      // Safety timeout — if worker hangs, reject cleanly
...
export default useCustomEngine;
